...
<div class="col-md-2">
		...
		<label class="control-label" style="margin-bottom: 5px;">Department</label>
            ...
            <option value="">Select</option>
							  ...
							  {
							  ?><option value="<?php echo $dept1['dept_id'];?>" <?php if($dept1['dept_id']==$dept_id){ echo "selected"; } ?>><?php echo $dept1['dept_name'];?></option>
                              <?php }?>
			...
			<select name="class_id" class="select2" onChange="return get_class_subject(this.value)" id="class_id">
                ...
                <?php
					foreach($class as $data){?>
						<option value="<?php echo $data['class_id']?>" <?php if($data['class_id']==$class_id){ echo "selected"; } ?>><?php echo $data['name']?></option>
					<?php } ?>
			</select>
            <?php
			}
    ...
    <div id="subject_holder">
        ...
        <div class="col-md-2">
            <div class="form-group">
                ...
                <select name="exam_id" class="select2" required>
                    <?php
...
<hr />
<?php echo form_close(); ?>
<!-- If internal mark is not needed --->
<?php
...
<div class="row">
                           ...
                           <?php echo form_open(base_url() . 'index.php/admin/marks_update1/' . $class_id . '/' . $section_id . '/' . $exam_id . '/' . $subject_id. '/' . $dept_id); ?>
               ...
               <input type="hidden" class="form-control selectboxit" name="comment" id="comment" value="">
                ...
                <table class="table table-bordered sortable">
                    <thead>
                        ...
                        <tr>
                           <th style="text-align: center;" class="table-header"> Roll No.&nbsp;&nbsp; <font color="white"> <i class="fa fa-sort" aria-hidden="true" title="Sort Roll Number"></i></font></th>
                            ...
                            <th style="text-align: center;" class="table-header">Student &nbsp;&nbsp; <font color="white"><i class="fa fa-sort" aria-hidden="true" title="Sort Name"></font></th>
                            <th style="text-align: center;" class="table-header">Internal Marks</th>
                            <th style="text-align: center;" class="table-header"><input type="text" style="width:60px;height:30px" placeholder="Out Of" id="int_total_mark_obtt" class="form-control"></th>
                            <th style="text-align: center;" class="table-header">Marks Obtained</th>
                            <th style="text-align: center;" class="table-header"><input type="text" style="width:60px;height:30px" placeholder="Out Of" id="total_mark_obtt" class="form-control"></th>
                    ...
                    <tbody>
						...
						$running_year=get_running_year();
                        $count = 1;
                       $this->db->select('s.name,m.mark_obtained,m.mark_total,m.mark_id,m.internal_marks,m.internal_total,e.roll');
					   $this->db->from('mark m');
			            $this->db->join('student s','s.student_id=m.student_id','LEFT');
			         $this->db->join('enroll e','e.student_id=m.student_id','LEFT');
					 $this->crud_model->check_student_status();
                        ...
                        foreach ($marks_of_students as $row):
                            ...
                            <tr>
                                ...
                                <td> 
                                    ...
                                    <input type="text" class="form-control" name="int_marks_obtained_<?php echo $row['mark_id']; ?>" tabindex="<?php echo $count; ?>"
                                           style="width:60px;height:30px" value="<?php echo $row['internal_marks']; ?>" onchange="return get_grade(this.value)">	
                                          
                                </td>
                                <td>
                                    <input type="text" class="form-control int_total_markk" name="int_mark_total_<?php echo $row['mark_id']; ?>"
                                           style="width:60px;height:30px" value="<?php echo $row['internal_total']; ?>">	
                                </td>
                                ...
                                <td>
                                    <input type="text" class="form-control" name="grade"
                                           style="width:60px;height:30px" value="<?php
													...
													$average = (( ($row['mark_obtained']+$row['internal_marks']) / ($row['mark_total']+$row['internal_total']) ) * 100);
                                                   //echo number_format($average, 2, '.', '');
													$p=$this->db->get('grade')->result_array();
													foreach($p as $res){
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#int_total_mark_obtt').change(function () {
            var tot_mark = $(this).val();
            var selected_seats = $("body .int_total_markk");
            $.each(selected_seats, function (key, value) {
                $(this).val(tot_mark);
            });
        });
