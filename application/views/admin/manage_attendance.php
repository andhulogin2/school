        ...
        <div class="page-content">
            ...
            <div class="row">
                ...
                <div id="section_holder">
                	<div class="col-md-2">
                		<div class="form-group">
                		    <label class="control-label" style="margin-bottom: 5px;">Time</label><br />
                			<select name="time">
                                <option value="morning" <?php if($time=='morning'){ echo "selected"; } ?>>Morning</option>
...
<hr />
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="white-box">
                        <?php echo form_open(base_url() . 'index.php/admin/attendance_update/'.$branch_id.'/'.$dept_id.'/' . $class_id . '/' . $section_id . '/' . $timestamp . '/' . $time); ?>
                		...
                		<div class="row" style="padding-left:50px;">
                			<div class="form-group">
                			  ...
                			  ?>
                                ...
                                <?php } ?>
                                <label class="switch switch-success"><input type="checkbox" name="phone2" id="phone2" value="1"><span></span> Send To Phone2? </label>
                                <?php
								if($this->db->get_where('settings',array('type'=>'voice_call'))->row()->description=='yes')
								{
									?>
                                	<label class="switch switch-success"><input type="checkbox" checked name="voice_call" id="voice_call" value="1"><span></span> Send Voice Call </label>
                                	<?php
								}
								?>
                			</div>
                        ...
                        <div class="row" style="padding-left:50px;padding-bottom:10px;">
                        	<div class="col-md-12">
                            	...
                            	<div class="col-md-1" style="height:20px;width:20px;background-color:#fb6161"></div><div class="col-md-1" style="">Absent</div>
                                <div class="col-md-1" style="height:20px;width:20px;background-color:#f7c260"></div><div class="col-md-1" style="">Late</div>
                                <div class="col-md-4"></div>
                            </div>    
                        </div>
                    <div>
                    ...
                    $count = 1;
					$afternoon_attendance	=	$this->db->get_where('settings',array('type'=>'afternoon_attendance'))->row();	
					$this->db->from('attendance a');
					$this->db->join('student b','b.student_id=a.student_id');
					$this->db->join('enroll c','c.student_id=a.student_id and c.year="'.$running_year.'"');
					...
					if($afternoon_attendance->description=='yes')
					{
						$this->db->where('a.time',$time);
					}	
					$this->db->order_by('c.roll','ASC');
					$this->db->order_by('b.name','ASC');
					$attendance_of_students = $this->db->get()->result_array();
                    ...
                    foreach ($attendance_of_students as $row){
                        ...
                        <tr <?php if($row['status'] == 2){?> style="background-color: #fb6161;color:white;" <?php }else if($row['status'] == 3){?> style="background-color: #f7c260;color:black;" <?php } ?>>
                            <td><?php echo $count++; ?></td>
                            <td>
                                <?php echo $row['roll'];//echo $this->db->get_where('enroll', array('student_id' => $row['student_id']))->row()->roll; ?>
                            </td>
                            <td>
                            ...
                            <div id="class1">
                                 <select class="form-control selectboxit" id="class" name="status_<?php echo $row['attendance_id']; ?>" onchange='Checklate(this.value,this.getAttribute("name"));'>
									...
									?>
                                     <?php if($this->db->get_where('settings' , array('type' =>'half_day_leave'))->row()->description == 'yes' && $this->db->get_where('settings' , array('type' =>'afternoon_attendance'))->row()->description != 'yes')
                                     {
			                         ?>
                                    <option value="5" <?php if ($row['status'] == 5) echo 'selected'; ?>>Half Day</option>
                                    <?php 
									} 
									?>
                                 </select>	
