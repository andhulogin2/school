                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <select name="receipt_number" class="select2" id="receipt_number" onChange="get_receipt(this.value)" >
                                  ...
                                  {
                                  ?><option value="<?php echo $row['receipt_number'].'_'.$row['student_id'];?>"><?php echo $row['receipt_number'].' ('.$row['name'].' - '.$row['class_name'].'/'.$row['section_name'].')';?></option>
                                  <?php }?>
...
<script type="text/javascript">
	...
	{
                var arr         =   receipt_number.split("_");
                var receipt_num =   arr[0];
                var student_id  =   arr[1]; 
		if(receipt_num!='')
		{
			$.ajax({
				url: '<?php echo base_url();?>index.php/FeeManagement/get_receipt_details/' + receipt_num +'/'+student_id,
