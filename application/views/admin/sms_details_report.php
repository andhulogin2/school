...
<div class="main-content">
				<div class="main-content-inner">
                   ...
                   <div align="right" style="padding-right:20px; padding-top:10px;"> 
                              ...
                              <a href="<?php echo base_url();?>index.php/admin/sms_report" data-dismiss="fileinput"><b><button class="btn-info">Back</button></b></a> 
                                   </div> 
					 <div align="right" style="padding-right:20px; padding-top:10px;"> 
           ...
           <table id="dynamic-table" class="table table-striped table-bordered table-hover" style="width:100%">
												<thead>
													<tr>
														...
														<th class=" table-header"><center>Phone</center></th>
                                                        <th class=" table-header"><center>Message</center></th>

												
														<th class=" table-header" width="10%"><center>Status</center></th>
                                                        <th class=" table-header" width="10%"><center>Resend</center></th>
             ...
             <tbody>
                                                  <?php 
												  $this->db->select('s.name as student,c.name as class,a.name as section,d.student_id,d.msg_content,d.phone,d.processed,d.status,d.details_id,m.send_date as send_date');
												 $this->db->join('student s','s.student_id=d.student_id','LEFT');
												  $this->db->join('class c','c.class_id=d.class_id','LEFT');
												   $this->db->join('section  a','a.section_id=d.section_id','LEFT');
												    $this->db->join('tbl_sms_delivery_master  m','d.sms_master_id=m.sms_master_id','LEFT');
												   
												  $this->db->where('d.sms_master_id',$master_id);
												  ...
												  $count = 1;foreach($query as $row){?>
													...
													<td><center>
														...
														if($row['class']==0)
														{
															$row['class']	=	get_student_class_name($row['student_id']);
														}
														if($row['section']==0)
														{
															$row['section']	=	get_student_section_name($row['student_id']);
														}
														?>
