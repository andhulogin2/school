...
<div class="col-sm-10 widget-container-col">
			...
			<?php echo form_open(base_url() . 'index.php/admin/voucher_update ' , array('id'=>'expense_form','class' => 'form-horizontal form-groups-bordered validate','target'=>'_top'));?>

<div align="right"><a href="<?php echo base_url().'index.php/admin/view_voucher'; ?>"><b>Back</b></a></div>						
          <div class="white-box">
            <br><br>
            
			<?php foreach($voucher as $row1)
			{	
			?>
            <input type="hidden" name="day_book_id" value="<?php echo $row1['day_book_id']; ?>"  />
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        ...
                        <div class="input-group input-group-sm">
                                <input type="text"  id="mydatepicker"  value="<?php echo date('d-m-Y',strtotime($row1['day_book_date'])); ?>" class="form-control mydatepicker" name="voucher_date"  required/>
				...
				<select name="branch" class="form-control selectboxit" id="branch" onChange="return get_dept(this.value)" required="">
                  <option value="">Select</option>
                   <?php $branch=$this->db->get('tbl_branch')->result_array();
					...
					{ 
					if($branch1['branch_id']==$row1['branch_id']) {?>
        ...
        <div class="form-group">
			...
			<div class="col-sm-5">
				<select name="department" class="form-control selectboxit" id="department" required="">
                  <option value="<?php echo $row1['department_id'] ?>"><?php echo get_dept($row1['department_id']); ?></option>
...
<?php } ?>
        ...
        <div class="col-sm-5">
			<select name="voucher_type" class="form-control selectboxit" id="voucher_type" class="col-xs-12 col-sm-12" required="" onchange="get_voucher_num(this.value); get_amount_type(this.value); get_item_head(this.value)" >
                ...
                <?php
				 ...
				 if($data['voucher_type_id']==$row1['voucher_type_id']) {?>
        ...
        <div class="col-sm-5">
			<select name="item_head" class="form-control selectboxit" id="item_head" class="col-xs-12 col-sm-12" required="">
                  <option value="">Select</option>
                     <?php
					    $this->db->where('department_id',$row1['department_id']);
                        if($this->session->userdata('account_section_id')!="1")
                        {
                            $this->db->where('account_section_id',$this->session->userdata('account_section_id')); 
                        }
                        if($row1['voucher_type_id'] == "1"){
							$this->db->where('account_type_id','4'); 
						}
                        if($row1['voucher_type_id'] == "2"){
							$this->db->where('account_type_id','3'); 
						}
                        if($row1['voucher_type_id'] == "3"){
							$this->db->where('account_type_id','1'); 
						}
                        $item_head  = $this->db->get('view_account_head')->result_array();
						foreach ($item_head as $row)
						{ 
						if($row['account_head_id']==$row1['account_head_id']) { ?>
						  <option value="<?php echo $row['account_head_id']; ?>" selected="selected"><?php echo $row['account_head_name']; ?></option>
						 <?php }
						 else
						 { ?>
						  <option value="<?php echo $row['account_head_id']; ?>"><?php echo $row['account_head_name']; ?></option>
						  <?php }
						   } ?>
             </select>
		</div>
        ...
        <div class="form-group">
		<label class="col-sm-4 control-label no-padding-right" for="form-field-1-1">Amount Type:<font color="#FF0000">* </font></label>
        <div class="col-sm-5">
			<input type="hidden" name="amount_types" id="amount_types" value="<?php if($row1['debit_amount']!="0") { echo "2"; } else { echo "1"; } ?>" />
            ...
            <select name="amount_type" class="form-control selectboxit" id="amount_type" class="col-xs-12 col-sm-12" required="" disabled="disabled" onchange="change_amount_type(this.value);"  >
				<option value="1" <?php if($row1['voucher_type_id']=="2" || $row1['credit_amount']!="0"){ echo "selected"; } ?> >Credit</option>
				<option value="2" <?php if($row1['voucher_type_id']=="1" || $row1['debit_amount']!="0"){ echo "selected"; } ?> >Debit</option>
			</select>
		</div>
	</div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-5">
                    <input type="text" class="form-control" value="<?php echo $row1['voucher_number'] ?>" name="voucher_number" id="voucher_number" placeholder="Voucher Number" required="" >
                </div>
            </div>
                  
            <div class="form-group">
		<label class="col-sm-4 control-label no-padding-right" for="form-field-1-1">Transaction Mode:<font color="#FF0000">* </font> </label>
        <div class="col-sm-5">
			<select name="transaction_mode" class="form-control selectboxit" id="transaction_mode" class="col-xs-12 col-sm-12" required="">
                ...
                <?php
				 ...
				 if($data['transaction_mode_id']==$row1['transaction_mode_id']) { ?>
				  <option value="<?php echo $data['transaction_mode_id']?>" selected="selected"><?php echo $data['transaction_mode']?></option>
                  <?php } else { ?>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
                 <input type="text" class="form-control" name="amount" id="amount" value="<?php  if($row1['credit_amount']!="0") {echo $row1['credit_amount']; } else {echo $row1['debit_amount']; } ?>" placeholder="Amount" required="">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
                <textarea  name="narration" style="width:350px;"><?php echo $row1['narration'] ?></textarea>
            </div>
        </div>
