            ...
            <table class="table simple-table table-bordered table-hover">   
                ...
                <tbody>
                	...
                	<tr name="rows[]">
                    	...
                    	<td>
                        	<center><?php echo $staff['name']; ?></center>
...
//Form validation
	...
	{	
		...
		if(disabled==student_checkbox.length)
		{
			...
			{
				alert("All staffs are already members of the selected group.");
			}
			if($('#add_remove').val()=='remove')
			{
				alert("Staffs are not members of the selected group.");
			}
			return false;
		}
		else if(checked==0)
	...
	{	
		...
		{	
			staff_id			=	staff[i].value;
			//student_checkbox[i].disabled	=	false;
			check_assigned1(academic_year_id,branch_id,students_group_master_id,staff_id,i);
			//alert(val['student_id']);
		}
	}
	function check_assigned1(academic_year_id,branch_id,students_group_master_id,staff_id,i)
	{
		
			$.ajax({
				url: '<?php echo base_url();?>index.php/Admin/check_staff_assigned/' + academic_year_id + '/' + branch_id + '/' + students_group_master_id + '/' + staff_id,
	...
	function disable_if_assigned(i,response)
	{
		...
		if(add_remove=='add')
		{
			...
			if(response==1)
			{
				...
				row[i].title					=	"Staff already assigned";
			}
			else
			{
				student_checkbox[i].disabled	=	false;
		...
		if(add_remove=='remove')
		{
			...
			{
				student_checkbox[i].disabled	=	true;
				row[i].title					=	"Staff not in the group";
			}
		}
	}
</script>
