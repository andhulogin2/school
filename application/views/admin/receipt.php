                        ...
                        <DIV align="center">
                            <div id="print">
                                <div class="box-body" id="printableArea">
                                    ...
                                    $class_name		 	= 	$this->db->get_where('class' , array('class_id' => $class_id,'academic_year' => $running_year))->row()->name;
                                    $section_name  		= 	$this->db->get_where('section' , array('section_id' => $section_id,'academic_year' => $running_year))->row()->name;
                                    //  $month_name         =  $month; 
                                        ...
                                        <table>
                                            ...
                                            <div style="padding-left:50px; padding-right:50px;">
                                                ...
                                                <table id="simple-table" class="table table-striped table-bordered table-hover"  cellpadding="2">
                                                    <tr>
                                                        <td style="text-align: left;width:50%">Receipt No. :<b><?php echo $receipt_no;?></b></td>
                                                        <td style="text-align: right;width:50%">Payment Date :<?php echo date('d-m-Y h:i A',strtotime($date_paid));?></td> 
                                                    </tr>
                                                    ...
                                                    <tr>
                                            <td style="text-align: left;">Class :<?php echo $class_name; ?>/<?php echo $section_name; ?></td>
                                            <?php $user_id = $this->db->get_where('tbl_fee_collection_master',array('receipt_number'=>$receipt_no))->row()->collected_by;?>
                                            <td style="text-align: right;">Collected by: <?php echo $this->db->get_where('staff',array('user_id'=>$user_id))->row()->name;?></td>
                                                    </tr>
                                                    <?php
												if($this->db->get_where('settings',array('type'=>'installments_row_in_receipt'))->row()->description == 'yes')
												{
                                                    ...
                                                    where a.receipt_number=".$receipt_no." and branch_id=".$branch_id." order by d.fee_payment_options_details";
                                                                        ...
                                                                        <table id="myTable" class="table table-striped" border="1">
                                                                            ...
                                                                            <tbody>
                                                                                ...
                                                                                $op_bal =   "(select a.id,a.opening_balance_id,a.amount_paid,b.fee_from_year_id,c.fee_head,d.academic_year ".
                                                                                            ...
                                                                                            "where a.receipt_number=".$receipt_no." and a.paid_year_id=".$running_year." and a.is_deleted='N') ".
                                                                                            "UNION ALL ".
                                                                                            ...
                                                                                            "join tbl_academic_year d on d.acdemic_year_id=b.fee_from_year_id ".
                                                                                            "where a.receipt_number=".$receipt_no." and a.paid_year_id=".$running_year." and a.is_deleted='N') order by fee_from_year_id asc";
                                                                                ...
                                                                                where c.fee_amount>0 and  c.fee_collection_master_id in ( select fee_collection_master_id from tbl_fee_collection_master where receipt_number =".$receipt_no." and branch_id=".$branch_id." and admission_number=".$student_id." ) GROUP BY c.fee_head_id ";
                                                                                /*
					...
					if($this->db->get_where('settings',array('type'=>'show_transport_fee_with_normal_fee_pay'))->row()->description == 'yes') 
					{
    					...
    					*/
    					...
    					$data =  $this->db->get('view_transport_students_bus_fee_collection_details')->row();	
                                                                                ...
                                                                                <tr>
                                                                                    <td colspan="3" align="center" >Amount in words :<b> Rupees <?php echo convert_number_to_words($total);  ?> Only</b></td>
                                                                                </tr>
                                                                            </tbody>
                                                                        </table>
...
$qry = "select c.fee_collection_master_id,c.fee_head_id,c.fee_amount ,h.fee_head,l.fee_payment_options_details from tbl_fee_collection_details as  c 
			...
			where c.fee_amount>0 and  c.fee_collection_master_id in ( select fee_collection_master_id from tbl_fee_collection_master where receipt_number =".$receipt_no." and branch_id=".$branch_id." ) order by  l.fee_payment_options_details_id asc ";
					...
					if($this->db->get_where('settings',array('type'=>'show_transport_fee_with_normal_fee_pay'))->row()->description == 'yes') 
					{
    					...
    					*/
    					...
    					$data =  $this->db->get('view_transport_students_bus_fee_collection_details')->result_array();	
                                                                                ...
                                                                                <tr>
                                                                                    <td colspan="4" align="center" >Amount in words :<b> Rupees <?php echo convert_number_to_words($total);  ?> Only</b></td>
                                                                                </tr>
                                                                            </tbody>
                                                                        </table>
