...
?>
 <th style="text-align: center;" class="text-nowrap"><a href="" data-toggle="modal" data-target="#modify_modal_<?php echo $data['students_fee_master_id']; ?>" > <i class="fa fa-edit text-info"></i>Modify </a>

     <!----------------------Modify fee Modal begins ------------------------->

  <div class="modal fade" id="modify_modal_<?php echo $data['students_fee_master_id']; ?>" role="dialog">
    <div class="modal-dialog" style="width:1100px">
    
      <!-- Modal content-->
      <div class="modal-content">
        <div class="modal-header" style="background-color:#52a4ca">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Modify Fee</h4>
        </div>
        <div class="modal-body">
        <p>
					 <?php echo form_open(base_url() . 'index.php/FeeManagement/student_fee_concession_update/', array('id'=>'modal_form','class' => 'form1', 'enctype' => 'multipart/form-data')); 
				 { ?>
                 <input type="hidden" name="students_fee_master_id1" value="<?php echo $data['students_fee_master_id']; ?>"  />
                <table id="simple-table" class="table table-striped table-bordered table-hover">
                	...
                	<?php
					$this->db->select('students_fee_details_id,students_fee_master_id,students_fee_details_id,fee_head_id,fee_amount,fee_balance,fee_concession');
					...
					?>
                    		<tr name="rows[]">
                                <td ><?php echo $i;?></td> 
                                <td ><?php echo get_fee_head_name($row['fee_head_id']);?></td> 
                                <td align="right" >
                                	<input type="hidden" name="students_fee_details_id1[]" id="students_fee_details_id1[]" value="<?php echo $row['students_fee_details_id'];?>" >
                                	<input type="text" name="total1[]" id="total1]" value="<?php echo $row['fee_amount'];?>" readonly >
                                </td> 
                                <td align="right" >
                                	<input type="text" name="paid1[]" id="paid1[]" value="<?php echo $fee_paid;?>" readonly >
                                </td>
                                <td  align="right" >
                                	<input type="text" name="concession1[]" id="concession1[]" value="<?php echo $row['fee_concession'];?>" <?php if($row['fee_balance']==0){echo "readonly";} ?> onChange="calculate_fee();" >
                                
                                </td> 
                                <td align="right" >
                                	<input type="text" name="balance1[]" id="balance1[]" value="<?php echo $row['fee_balance'] ;?>" readonly />
                                </td>
                                <td align="right" >
                                	<input type="text" name="remarks[]" id="remarks[]" />
                                </td>
                                <td align="center" >
                                	<input type="checkbox" name="fee_head_balance_check1[]" id="fee_head_balance_check1[]" onChange="calculate_fee();" <?php if($row['fee_balance']==0){echo "disabled";} ?> >
                                    <input type="hidden" id="check_uncheck2[]" name="check_uncheck2[]" onChange="calculate_fee();">
                                </td>
                            </tr>
                    <?php
						$i=$i+1;	
					endforeach;
					?>
                                    <input type="hidden" id="total_balance1" name="total_balance1">
                                    <input type="hidden" id="total_concession1" name="total_concession1">
                            <tr>
                                <td colspan="8" align="center">
                                <button type="button" class="btn btn-success" name="btn_pay_now1" id="btn_pay_now1" onClick="return validate_form();"><b>Update</b>   </button>
                                </td>
                            </tr>
                    	</tbody>
                    </table>
...
var count_item = fee_item_balance.length;
  ...
  for (var i = 0;  i < balance.length; i++)
   ...
   if(chkbalance[i].checked)
	...
	{ 
		var total 					=  	document.getElementsByName('total1[]');
		var fee_head_balance_check 	=  	document.getElementsByName('fee_head_balance_check1[]');
	...
	function calculate_fee() 
	{
		...
		var total 					=  	document.getElementsByName('total1[]');
		var balance 				=  	document.getElementsByName('balance1[]');
		var concession 				=  	document.getElementsByName('concession1[]');
		var fee_head_balance_check 	=  	document.getElementsByName('fee_head_balance_check1[]');
		var check_uncheck 			=  	document.getElementsByName('check_uncheck2[]');
		for(var i = 0;  i < total.length; i++)
		{
			if(fee_head_balance_check[i].checked)
			{
				...
				{
					balance[i].value 		=	parseFloat(parseFloat(total[i].value).toFixed(2) - parseFloat(concession[i].value).toFixed(2)).toFixed(2);
					concession[i].value		=	parseFloat(fee_concession1).toFixed(2);
					check_uncheck[i].value	=	1;
					document.getElementById('btn_pay_now1').disabled = false;
				}
			}
			else
			{
			...
			document.getElementById("total_balance1").value		=	total_balance;
			document.getElementById("total_concession1").value	=	total_concession;
		}   
		//document.getElementById('amount').value=total;
    }
