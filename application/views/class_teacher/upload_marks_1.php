...
<?php echo form_open(base_url() . 'index.php/class_teacher/marks_selector');?>
	<div class="col-md-2">
		<div class="form-group">
		<label class="control-label" style="margin-bottom: 5px;">Class <?PHP ECHO $admin;?></label>
			<select name="class_id" class="form-control selectboxit" onchange="get_class_subject(this.value)">
				<option value="">Select</option>
				<?php
				$class_teacher_id=$this->db->get_where('staff' ,array('user_id'=>$admin))->row()->staff_id;
                       $this->db->select('class_id ,name');
					    $this->db->where('teacher_id',$class_teacher_id);
					   
						$class=$this->db->get('view_class_teacher')->result_array();
...
<script type="text/javascript">
	function get_class_subject(class_id) {
		...
		$.ajax({
            ...
            url: '<?php echo base_url();?>index.php/class_teacher/marks_get_subject/' + class_id ,
