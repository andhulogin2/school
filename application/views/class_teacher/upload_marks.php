	...
	<div class="col-md-2">
		...
		<label class="control-label" style="margin-bottom: 5px;">Class</label>
		    <?php
				...
				$class=$this->db->get('class c')->result_array();//echo $this->db->last_query();die();
		    ?>
			<select name="class_id" class="select2" onchange="get_class_subject1(this.value)">
				...
				/*$teacher_id=$this->db->get_where('staff' ,array('user_id'=>$admin))->row()->staff_id;
					   ...
					   $this->db->where('d.academic_year',$this->session->userdata('academic_year'));
...
<script type="text/javascript">
	function get_class_subject1(class_id) {
		...
		$.ajax({
            ...
            url: '<?php echo base_url();?>index.php/Class_teacher/marks_get_subject_myclass/' + class_id ,
