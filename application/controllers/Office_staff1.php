...
class Office_staff extends CI_Controller {
	...
	function attendance_selector()
    {
     ...
     redirect(base_url().'index.php/admin/manage_attendance/'.$data['branch_id'].'/'.$data['dept_id'].'/'.$data['class_id'].'/'.$data['section_id'].'/'.$data['timestamp'],'refresh');
 ...
 $location = 'uname='.urlencode($username).'&pwd='.urlencode($password).'&senderid='.urlencode($sender_id).'&to='.$phone1.','.$phone2.'&msg=' .urlencode(" ". $message." ".$common).'&route=T';
	...
	redirect(base_url() . 'index.php/admin/view_subject/'. $data['class_id'].'/'.$branch_id.'/'.$dept_id, 'refresh'); 
	}       
	function edit_class()
	{
...
function expense_category()	
	...
	function expense_add()	
	{
		...
		$this->load->view('admin/view_expense.php');
	}
