...
echo  "<table border='0'><tr><td colspan='2' align='center'></td><td colspan='2' align='center'></td><td colspan='3' align='center'></td></tr>";
	...
	$running_year = get_running_year();
		...
		/////////////  Insert Data to Fee Master If Fee Plan is Chosen
		...
		$data3['action']="success";
          ...
          //redirect(base_url() . 'index.php/admin/students_area/'.$data2['class_id']);
		  $this->load->view('admin/add_student.php',$data3);
	}
	
///////////////////////////// this funaction is called when a student is taking admission, ie from admin/add_student
...
/////////////////////////////////////////////////
	...
	function add_staff()
    {
           ...
           $password   = $this->input->post('password');
            ...
            $staff_id =$this->crud_model->staff_insert($name,$username,$salary,$sex,$address,$phone,$email,$password);
	...
	{
		$this->load->view('admin/student_area.php');
	 ...
	 function search($search_key = '') 
    {
	 ...
	 //  redirect(base_url() . 'index.php/admin/search/'. $search_key, 'refresh');
    }
 ...
 $location = 'uname='.urlencode($username).'&pwd='.urlencode($password).'&senderid='.urlencode($sender_id).'&to='.$phone1.','.$phone2.'&msg=' .urlencode(" ". $message." ".$common).'&route=T';
	...
	function attendance_messages($class_id,$section_id,$student_id,$present,$total,$percentage,$month)
    {
       ...
       $phone2 = $this->db->get_where('student' , array('student_id' => $student_id))->row()->phone2;
		...
		if($balance >= 0){
           ...
           redirect(base_url() . 'index.php/admin/report_attendance_view/'.$class_id.'/'.$section_id.'/'.$month1,'refresh');
    }
	...
	function create_exam($param1 = '', $param2 = '' , $param3 = '')
    {
	  $page_data['exams']      = $this->db->get('exam')->result_array();
        ...
        if ($param1 == 'delete') {
          ...
          if ($param1 == 'new') 
        {
            $this->load->view('admin/create_exam', $page_data);
        }
	...
	function marks_selector()
    {
	  ...
	  if($query->num_rows() < $query1->num_rows()) 
        ...
        redirect(base_url() . 'index.php/admin/marks_upload/' . $data['class_id'] . '/' . $data['section_id'] . '/' . $data['exam_id'] . '/' . $data['subject_id'] , 'refresh');
    }
 ...
 function tab_sheet_print($class_id ,$section_id, $exam_id) {
	...
	function homework($param1 = '', $param2 = '') 
    {
        ...
        if ($param1 == 'edit') 
        {
            ...
            redirect(base_url() . 'index.php/admin/homeworkroom_edit/edit/' . $param2 , 'refresh');
        }
        if ($param1 == 'delete'){
          ...
          $api = $url;
            ...
            if ($balance >= 0) {
                ...
                $message_id_array = explode(",", $return_message_ids);
						  ...
						  alert("Message Send Successfully")</script><?php 


			  				redirect(base_url() . 'index.php/admin/message' , 'refresh');


			  		//$this->load->view('admin/message.php',$page_data);
...
//" http://domainname/sendunicodesms?uname=yourUsername&pwd=yourPassword&senderid=yourSenderid&to=9444xxxxxx&msg=yourunicodeMessage&route=yourRoute " 
			...
			//	die();
			  ...
			  ?><script>alert("Message Send Successfully")</script><?php
			  				redirect(base_url() . 'index.php/admin/message/malayalam_message' , 'refresh');
	 ...
	 function class_add()
	{
	 ...
	 function view_subject($class_id= '')
	{
	...
	function view_class()
	{
	 ...
	 function new_subject_add($class_id)
	{
	$page_data['class_id'] =$class_id;
	...
	$class_name['class_id']=$class_id;
	 ...
	 function subject_edit($class_id,$subject_id,$teacher_id)
	{
	$data['class_id']=$class_id;
	...
	$data['teacher_id']=$teacher_id;
	 ...
	 function update_subject($class_id)
	{  
	        $data['class_id']	=$this->input->post('class_id');
           ...
           redirect(base_url() . 'index.php/admin/view_subject/'. $data['class_id'], 'refresh'); 
     }       
	function edit_class()
	{
	 ...
	 function student_bulk1($param1 = '')
    {
        ...
        //$page_data['page_title'] = get_phrase('Student-Bulk');
					$dat["action"]="success";
					        ...
					        $this->load->view('admin/student_bulk',$dat);
